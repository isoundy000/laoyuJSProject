...
(function (exports) {
    ...
    var TEXTURE_STATUS = {
        'kan': cc.textureCache.addImage("res/image/ui/pdk_jbc/unit/yikanpai.png"),
        'qi': cc.textureCache.addImage("res/image/ui/pdk_jbc/unit/yiqipai.png"),
        'shu': cc.textureCache.addImage("res/image/ui/pdk_jbc/unit/bipaishu.png")
    };
    var BatteryTextures = {
    ...
    var TEXT_2_FILENAME = {};
    TEXT_2_FILENAME[TEXT_BUYAO] = "res/image/ui/pdk_jbc/character/text_buchu.png";
    TEXT_2_FILENAME[TEXT_YAOBUQI] = "res/image/ui/pdk_jbc/character/text_yaobuqi.png";
    TEXT_2_FILENAME[TEXT_DAWANLA] = "res/image/ui/pdk_jbc/character/text_dawanla.png";
    ...
    var SHENFEN_TEXTURE = {};
    SHENFEN_TEXTURE["dizhu_l"] = cc.textureCache.addImage("res/image/ui/pdk_jbc/character/dizhu_l.png");
    SHENFEN_TEXTURE["dizhu_r"] = cc.textureCache.addImage("res/image/ui/pdk_jbc/character/dizhu_r.png");
    SHENFEN_TEXTURE["nongmin_l"] = cc.textureCache.addImage("res/image/ui/pdk_jbc/character/nongmin_l.png");
    SHENFEN_TEXTURE["nongmin_r"] = cc.textureCache.addImage("res/image/ui/pdk_jbc/character/nongmin_r.png");

    var LAIZI_TEXTURE = cc.textureCache.addImage("res/image/ui/pdk_jbc/character/lai_star.png");
    ...
    var PokerJBCLayer = cc.Layer.extend({
        chatLayer: null,
        chiLayer: null,
        throwDiceLayer: null,
        kaigangLayer: null,
        ...
        reNew: function (isNew) {
            this.roomReNew = isNew;
        },
        isReNew: function () {
            return this.roomReNew;
        },
        ...
        onCCSLoadFinish: function () {
            ...
            TouchUtils.setOnclickListener($('setting'), function () {
                ...
                setting.setSetting(that, "pdk_jbc");//大厅里面打开界面
            ...
            network.addListener(3002, function (data) {
                ...
                //         gameData.totalRound = data['total_round'];
                //         gameData.playTime = data['play_time'] || 15;
                //     }
                //     // var wanfa = $("word_wanfa_" + gameData.mapId);
                //     // if (wanfa)
                //     //     wanfa.setVisible(true);
                // }
                console.log('getRoomState...', that.getRoomState());

                if (that.isReNew()) {
                    var layer = that.getChildByName('pokerJBCJieSuanLayer');
                    ...
                    console.log('onPlayerEnterExit...', that.getRoomState());
            ...
            network.addListener(3003, function (data) {
                ...
                var exit_room_op = data["exit_room_op"];
                if (exit_room_op) {
                    if (exit_room_op == 3) {
                        var msg = '您的金币和当前场次不匹配，系统已自动为您匹配';
                        that.reNew(true);
                        that.scheduleOnce(function () {
                    ...
                    } else if (exit_room_op == 4) {
                        that.reNew(true);
                        that.scheduleOnce(function () {
                            network.send(99998, { cmd: 'addEncourageCoin', area: 'niuniu' });
                        }, 3);
                    } else if (exit_room_op == 1) {
                        that.reNew(true);
                        console.log('超时，已经退出房间');
                    } else if (exit_room_op == 6) {
                ...
                network.send(99999, { cmd: 'userOut', area: gameData.parent_area });
            });
            network.addListener(3230, function (data) {
                ...
                if ((data['errorCode'] == -8) || (data['errorCode'] == -7) || (data['errorCode'] == -6)) {
                    alert1(data['errorMsg'], function () {
                        gameData.enterJBC = true;
                        HUD.showScene(HUD_LIST.Home, null);
                    });
                }
            });
            network.addListener(3233, function (data) {
                if (data['errorCode'] == -10) {
                    // 强制退回
                    gameData.enterJBC = true;
                    HUD.showScene(HUD_LIST.Home, null);
                    ...
                    alert1(data['errorMsg']);
                } else if ((data['errorCode'] == -8) || (data['errorCode'] == -7) || (data['errorCode'] == -6)) {
                    alert1(data['errorMsg'], function () {
                        gameData.enterJBC = true;
                        HUD.showScene(HUD_LIST.Home, null);
                    });
            ...
            network.addListener(3004, function (data) {
                ...
                //     return;
                var uid = data["uid"];
                if (uid) that.setReady(uid);
            });
            network.addListener(3013, function (data) {
                var numOfCards = data["numof_cards"];
                var uid = data["uid"]
                console.log('3013=>', numOfCards);
                console.log('3013=>', uid);
                if (numOfCards && numOfCards.length > 0) {
                    if (uid) {
                        console.log('info=>', uid2position[uid]);
                        $("info" + uid2position[uid] + ".lb_score").setString(equalNum(numOfCards[0]));
                    } else {
                        $("info" + uid2position[gameData.uid] + ".lb_score").setString(equalNum(numOfCards[0]));
                    }
                }
            });
            ...
            network.addListener(4503, function (data) {
                ...
                if (!isReplay) {
                    if (row == 2) {
                        if (cardtype && cardtype != '') {
                            ...
                            $('row2.cardtype').loadTexture('res/image/ui/pdk_jbc/character/cardtype' + cardtype + '.png');
                        } else {
                ...
                if (QI) {
                    ...
                    for (var i = 0; i < 3; i++) {
                        $('row' + row + 'b.b' + i).setTexture('res/image/ui/pdk_jbc/unit/pai_backold.png');
                    }
                    that.wordsFly(row, '弃牌');
                }
                if (GEN) {
                    //筹码动画
                ...
                if (JIA) {
                    ...
                    if (!isReplay) {
                        $('ops.gen.txt').setString('跟注x' + data.genchip);
                    }
                }
                if (BI) {
                    ...
                    that.biPaiAnim(row, uid2position[data['win_uid']], uid2position[data['lose_uid']], function () {
                        ...
                        for (var i = 0; i < 3; i++) {
                            $('row' + uid2position[data['lose_uid']] + 'b.b' + i).setTexture('res/ui/resources/unit/pai_backold.png');
                        }
                        if (!isReplay) {
                            ...
                            if (_.isArray(data['pai_arr']) && data['pai_arr'].length > 0) {
                                ...
                                $('row2.cardtype').setVisible(true);
                                $('row2.cardtype').loadTexture('res/image/ui/pdk_jbc/character/cardtype' + data.card_type + '.png');
                                if (uid2position[data['lose_uid']] == 2) {
                                    setTimeout(function () {
                ...
                if (KAN) {
                    if (!isReplay) {
                        if (row == 2) {
                            ...
                            $('ops.gen.txt').setString('跟注x' + data['needchip'])
                        }
                    }
            ...
            network.addListener(4504, function (data) {
                ...
                for (var i = 0; i < data['canbi_arr'].length; i++) {
                    ...
                    $('info' + row + '.head_bg_anmi').setVisible(true);
                    (function (idx) {
                        TouchUtils.setOnclickListener($('info' + uid2position[data['canbi_arr'][idx]]), function () {
            ...
            network.addListener(4200, function (data) {
                if (mapId != MAP_ID.ZJH) {
                    ...
                    if ((mapId == MAP_ID.PDK_JBC) && uid2position[zhuangID] != 2) {
                    ...
                    if (!isReplay) {
                        for (var i = 1; i < 6; i++) {
                            for (var j = 0; j < 3; j++) {
                                $('row' + i + 'b.b' + j).setTexture('res/ui/resources/unit/pai_back.png');
                            }
                        }
                        gameData.totalRound = data["total_round"];
                    }
                    ...
                    for (var i = 0; i < gameData.players.length; i++) {
                        $("row" + uid2position[gameData.players[i].uid] + 'b').setVisible(true);
                        $("row" + uid2position[gameData.players[i].uid]).setVisible(false);
                    }
                    for (var i = 1; i < 6; i++) {
                        that.updateCardStatusZJH(i);
                    }
                    that.faAllPai(function () {
                                ...
                                if (data.pai_arr.hasOwnProperty(_uid) && _uid && _uid != '0') {
                                    ...
                                    if (_row) {
                                        ...
                                        (function (__row, __uid) {
                                            ...
                                            $('row' + __row + '.cardtype').loadTexture('res/image/ui/pdk_jbc/character/cardtype' + data.pai_arr[__uid].cardtype + '.png');
                                        })(_row, _uid);
                                    }
                                }
                            }
            ...
            network.addListener(99998, function (data) {
                ...
                if (errorCode == 0) {
                    ...
                    console.log('currentGoldNum', coins);
                    // $('root.coin.lb_coin').setString(coins[0]);
                    var level = window.jbcMainLayer.getCurLevel();
                    var minGold = window.jbcMainLayer.getCurMin();

                    network.send(3013);
                    alert1('您的金币不足，系统免费赠送您' + data['perchipnum'] + "金币", function () {
                        that.reNew(true);
                        // if (gameData.numOfCards[0] >= minGold) {
                        // network.send(3230, { gold_map_id: MAP_ID.PDK_JBC, gold_room_lev: gameData.gold_room_lev });
                        // }
                    });
                } else {
                    HUD.showConfirmBox('提示', '当前金币不足，是否充值？', function () {
                        gameData.enterJBC = true;
                    ...
                    // alert2('当前金币不足，且无法领取鼓励金，是否充值？', function () {
                    //     if (window.mScene) window.mScene.clickShop(2);
                    // }, function () {
                    //     gameData.enterJBC = true;
                    //     HUD.showScene(HUD_LIST.Home, null);
            ...
            network.send(99999, { cmd: 'userIn', area: gameData.parent_area });
        ...
        changeBg: function (sceneid) {
            ...
            $('bg').setTexture('res/image/ui/pdk_jbc/table/table_back' + sceneid + '.jpg');
            // $('bg').setTexture('res/submodules/pdk_jbc/image/table_back' + sceneid + '.jpg');
        },
        ...
        onPlayerEnterExit: function () {
            console.log('player enter...', roomState);
            var that = this;
            ...
            for (var i = 0, j = 2; i < players.length; i++ , j++) {
                ...
                $("info" + k + ".lb_score").setString(equalNum(player["goldNum"]));//((player["total_score"] == undefined) ? player["score"] : player["total_score"]));
        ...
        ctor: function (_data, _isReplay) {
            ...
            loadNodeCCS(res.PkScene_jbc_json, this, "Scene");
            // }
        ...
        throwTurn: function (row, data) {
            ...
            if (gameData.map_id == MAP_ID.PDK_JBC) {
                $('timer2').setVisible(false);
        ...
        setPaiHua: function (pai, val, row) {
            ...
            if (paiid == 0) {
                ...
                if (val == -1) {
                    pai.setTexture('res/submodules/pdk_jbc/image/pai_back3.png');
                } else {
                    var arr = this.getPaiNameById_PDK(val);
                    setSpriteFrameByPath(pai, arr, 'res/submodules/pdk_jbc/image/newpoker.plist');
                }
        ...
        recalcPos: function (row, paiCnt) {
            ...
            $("row" + row).setPositionX((cc.winSize.width - width) / 2 - (cc.winSize.width - 1280) / 2);
        },
        ...
        setRoomState: function (state) {
            ...
            gameData.pdk3Abomb = arr.join(",").indexOf("3A当炸弹") != -1;
            gameData.pdk3A1bomb = arr.join(",").indexOf("3A带1张当炸弹") != -1;
            ...
            if (state == ROOM_STATE_CREATED) {
                // var okSign = $("info" + uid2position[gameData.uid] + ".ok");
                // if(okSign) okSign.setVisible(false);
            ...
            if (state == ROOM_STATE_ONGOING) {
                ...
                $("btn_zhunbei").setVisible(false);
                $('btn_ready').setVisible(false);
                $("btn_mic").setVisible(!window.inReview && gameData.mapId != MAP_ID.PDK_JBC);
                $("lb_roomid").setString(gameData.roomId);
                $("lb_wanfa").setString(wanfaStr);
                //$("lb_roomid").setVisible(false);
                $("row1").setVisible(false);
                ...
                $('bsks').setVisible(false);
                network.send(3013);//匹配成功扣除服务费请求刷新
            ...
            if (gameData.mapId == MAP_ID.PDK_JBC) {
                ...
                $('timer4.Text_1').setString(gameData.gold_room_name);
                $('timer4.Text_2').setString('底分：' + gameData.gold_room_base);

                $('timer4').setVisible(true);
                $("lb_roomid").setVisible(false);
                $("lb_roomid_").setVisible(false);
                $("imgbg_roomid").setVisible(true);
                $("btn_mic").setVisible(false);
        ...
        // getLevel: function () {
        //     var str = ['新手场', '初级场', '中级场', '高级场'];
        //     if (gameData.gold_room_lev && str[gameData.gold_room_lev - 1]) {
        //         return str[gameData.gold_room_lev - 1];
        //     } else {
        //         return '';
        //     }
        // },
                            ...
                            if (row == 2) {
                                if (playerPai['read']) {
                                    ...
                                    if (playerPai['isloser'] || playerPai['isqi']) {
                                        $('row2b').setVisible(true);
                                        for (var j = 0; j < 3; j++) {
                                            $('row' + row + 'b.b' + j).setTexture('res/image/ui/pdk_jbc/unit/pai_backold.png');
                                        }
                                        that.updateCardStatusZJH(2, playerPai['isqi'] ? 'qi' : 'shu');
                                    }
                                }
                ...
                if (roomState == ROOM_STATE_ONGOING) {
                    if (mapId != MAP_ID.ZJH) {
                        ...
                        for (var chipnum = 0; chipnum < data['chips'].length; chipnum++) {
                            (function (chipnum) {
                                var chip = new cc.Sprite(cc.textureCache.addImage('res/image/ui/pdk_jbc/unit/chip' + data['chips'][chipnum] + '.png'));
                                chip.setPosition(Math.random() * 300 + 490, Math.random() * 270 + 330);
        ...
        enableChuPai: function () {
            ...
            var chupaiListener = cc.EventListener.create({
                ...
                onTouchEnded: function (touch, event) {
                    ...
                    if (slideOverCnt > 0) {
                        //关闭优先提示顺子
                        if(gameData.opt_conf.pdkchupaitishi && pokerRule && typeof pokerRule.automaticTip == 'function'){
                            //有之前滑起的牌  ，全部下去,重新选择
                            var upbefore = false;
                            for (var i = 0; i < initPaiNum; i++) {
        ...
        jiesuan: function (data) {
            ...
            for (var i = 0; i < players.length; i++) {
                ...
                $("info" + uid2position[uid] + ".lb_score").setString(equalNum(player["total_gold"]));
                ...
                //             $('row' + row + '.cardtype').setVisible(true);
                //             $('row' + row + '.cardtype').loadTexture('res/ui/resources/character/cardtype' + player['card_type'] + '.png');
                //         } else {
                //             $('row' + row + '.cardtype').setVisible(false);
                //         }
                //     }
                ...
                //         setTimeout(function () {
                //             if (_player.score != 0) {
                //                 that.wordsFly(_row, (_player.score > 0 ? '+' : '') + _player.score, 2, 1.5);
                //             }
            ...
            //                     cc.moveTo(0.4 + Math.random() * 0.3, endPos),
            //                     cc.callFunc(function () {
            ...
            setTimeout(function () {
                ...
                switch (gameData.mapId) {
                    ...
                    case MAP_ID.PDK_JBC:
                        ...
                        //console.log("PokerJBCJieSuanLayer" + isChooseQiepai);
                        layer = new PokerJBCJieSuanLayer(data, isReplay, !!isChooseQiepai, this);
                        layer.setName('pokerJBCJieSuanLayer');
                        isChooseQiepai = false;
                        break;
                    case MAP_ID.DDZ_JD:
        ...
        openJiesuanLayer: function () {
            ...
            setTimeout(function () {
                ...
                switch (gameData.mapId) {
                    ...
                    case MAP_ID.PDK_JBC:
                    case MAP_ID.PDK_MATCH:
                        layer = new PokerJBCJieSuanLayer(that.jieSuanData);
                        break;
                    case MAP_ID.DDZ_JD:
        ...
        zongJiesuan: function (data, delay) {
            ...
            setTimeout(function () {
                ...
                if (mapId == MAP_ID.ZJH) {
                    ...
                    var layer = new PokerJBCZongJiesuanLayer(data);
                    that.addChild(layer);
                }
            }, delay || 1);
        ...
        showChat: function (row, type, content, voice, uid) {
            ...
            if (!scale9sprite) {
                scale9sprite = new cc.Scale9Sprite("res/submodules/pdk_jbc/image/common/ltqp" + (row == 4 ? 3 : (row % 4)) + ".png", posConf.ltqpRect[row], posConf.ltqpCapInsets[row]);
        ...
        initQP: function (row) {
            ...
            var scale9sprite = $('info' + row + '.qp9');
            if (!scale9sprite) {
                scale9sprite = new cc.Scale9Sprite('res/submodules/pdk_jbc/image/common/ltqp' + row + '.png', posConf.ltqpRect[row], posConf.ltqpCapInsets[row]);
                scale9sprite.setName('qp9');
                //scale9sprite.setAnchorPoint(row == 1 ? cc.p(1, 0) : cc.p(0, 0));
                scale9sprite.setAnchorPoint(row == 1 ? cc.p(1, 0) : (row == 0 ? cc.p(1, 0) : cc.p(0, 0)));
                scale9sprite.setPosition(posConf.ltqpPos[row]);
        ...
        biPaiAnim: function (fromRow, row1, row2, cb) {
            ...
            var cards = {};
            for (var i = 1; i <= 2; i++) {
                cards[i] = duplicateLayout($(i == 1 ? 'row3b' : 'row1b'));
                for (var j = 0; j < 3; j++) {
                    cards[i].getChildByName('b' + j).setVisible(true);
                    cards[i].getChildByName('b' + j).setTexture('res/image/ui/pdk_jbc/unit/pai_back.png');
                }
                cards[i].getChildByName('status').setVisible(false);
                cards[i].setVisible(false);
                cards[i].setScale(0.5);
                cards[i].setPosition(i == 1 ? 360 : 920, i == 1 ? 432 : 430);
                that.addChild(cards[i], 2);
                var row = (i == 1 ? fromRow : (loser == 1 ? row1 : row2));
                info[i] = ccs.load(res.VS_json).node;
                ...
                (function (k) {
                    setTimeout(function () {
                        info[k].runAction(cc.sequence(
                            ...
                            cc.delayTime(0.3),
                            cc.spawn(
                                cc.moveTo(0.4, k == 1 ? 160 : 1120, k == 1 ? 447 : 447).easing(cc.easeIn(3)),
            ...
            setTimeout(function () {
                var bi_sp = playSpAnimation('vs', 'vs', false);
                bi_sp.setPosition(640, 450);
                bi_sp.setTimeScale(0.5);
                setTimeout(function () {
                ...
                setTimeout(function () {
                    for (var i = 0; i < 3; i++) {
                        ...
                        cards[loser].getChildByName('b' + i).setTexture('res/ui/resources/unit/pai_backold.png');
                    }
                    setTimeout(function () {
                        for (var i = 1; i <= 2; i++) {
        ...
        faAllPai: function (cb) {
            ...
            var func = function (i, j, idx) {
                ...
                sprite.setTexture('res/image/ui/pdk_jbc/unit/pai_back.png');
        ...
        disableBtn: function (btnname) {
            $(btnname).setTexture('res/image/ui/pdk_jbc/button/btn_disable.png');
        ...
        enableBtn: function (btnname) {
            var texture_filename = {
                'ops.qi': 'res/submodules/pdk_jbc/image/btn_hong.png',
                'ops.bi': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'ops.jia': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'ops.gen': 'res/submodules/pdk_jbc/image/btn_cheng.png',
                'jia.jia2': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'jia.jia3': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'jia.jia4': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'jia.jia5': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'ops.buchu': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'ops.chongxuan': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'ops.tishi': 'res/submodules/pdk_jbc/image/btn_cheng.png',
                'ops.chupai': 'res/submodules/pdk_jbc/image/btn_cheng.png',
                'btn_jiao1': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'btn_jiao2': 'res/submodules/pdk_jbc/image/table/btn_lan.png',
                'btn_jiao3': 'res/submodules/pdk_jbc/image/btn_lan.png',
                'btn_buyao': 'res/submodules/pdk_jbc/image/btn_cheng.png'
            };
            var outline_color = {
    ...
    exports.PokerJBCLayer = PokerJBCLayer;
})(window);
