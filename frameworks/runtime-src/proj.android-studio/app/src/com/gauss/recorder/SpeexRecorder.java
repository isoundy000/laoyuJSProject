...
public class SpeexRecorder implements Runnable {
		...
		for (int rate : mSampleRates) {
			for (short audioFormat : new short[] { AudioFormat.ENCODING_PCM_16BIT }) {
				for (short channelConfig : new short[] { AudioFormat.CHANNEL_IN_MONO }) {
					try {
						Log.d("SpeexRecorder", "Attempting rate " + rate + "Hz, bits: " + audioFormat + ", channel: "
								+ channelConfig);
						...
						Log.e("SpeexRecorder", rate + "Exception, keep trying.",e);
					}
				}
			}
		}
	...
	public void run() {
		...
		frequency = findSampleRate();

		// 启动编码线程
		SpeexEncoder encoder = new SpeexEncoder(this.fileName, frequency);
