...
cc.game.onStart = function () {
    ...
    if (cc.sys.isNative) {
        ...
        cc.sys.writablePath = path;

        var searchPaths = jsb.fileUtils.getSearchPaths();
        var newSearchPaths = [cc.sys.writablePath];
        for (var i = 0; i < searchPaths.length; i++)
            newSearchPaths.push(searchPaths[i]);
        //cc.log(JSON.stringify(newSearchPaths));
        jsb.fileUtils.setSearchPaths(newSearchPaths);
    }

    if (!cc.sys.isNative) {
        cc.view.setDesignResolutionSize(1560, 720, cc.ResolutionPolicy.SHOW_ALL);
    } else if (cc.winSize.width / cc.winSize.height === 16 / 9) {
    ...
    // cc.view.setRealPixelResolution(640, 1136, cc.ResolutionPolicy.SHOW_ALL);
    // The game will be resized when browser size change
    cc.view.resizeWithBrowserSize(true);
    //load resources
    cc.LoaderScene.preload(g_resources, function () {
        cc.loader.loadJs(["src/common/utils.js", "src/UpdateScene.js"], function (err) {
