...
class CCPluginCompile(cocos.CCPlugin):
    ...
    def _add_custom_options(self, parser):
        ...
        group.add_argument("--tizen-arch", dest="tizen_arch", default="x86", choices=["x86", "arm"],
                           help=MultiLanguage.get_string('COMPILE_ARG_TIZEN_ARCH'))
        ...
        group.add_argument("--tizen-profile", dest="tizen_profile",
                           help=MultiLanguage.get_string('COMPILE_ARG_TIZEN_PROFILE'))
        group.add_argument("--tizen-sign", dest="tizen_sign", help=MultiLanguage.get_string('COMPILE_ARG_TIZEN_SIGN'))
        group.add_argument("--tizen-strip", dest="tizen_strip", action="store_true",
                           help=MultiLanguage.get_string('COMPILE_ARG_TIZEN_STRIP'))
    ...
    def build_android(self):
        ...
        if using_studio:
            ...
            ide_name = 'Eclipse'
        cocos.Logging.info(
            MultiLanguage.get_string('COMPILE_INFO_ANDROID_PROJPATH_FMT', (ide_name, project_android_dir)))
        ...
        if not self._project._is_script_project() or self._project._is_native_support():
            if self._ndk_mode != "none" and not gradle_support_ndk:
                ...
                self._project.invoke_custom_step_script(cocos_project.Project.CUSTOM_STEP_PRE_NDK_BUILD,
                ...
                self._project.invoke_custom_step_script(cocos_project.Project.CUSTOM_STEP_POST_NDK_BUILD,
                                                        target_platform, args_ndk_copy)

        # build apk
        if not self._no_apk:
            cocos.Logging.info(MultiLanguage.get_string('COMPILE_INFO_BUILD_APK'))
        self.apk_path = builder.do_build_apk(build_mode, self._no_apk, output_dir, self._custom_step_args, self._ap,
                                             self)
    ...
    def build_ios(self):
        ...
        try:
            ...
            command = ' '.join([
                ...
                "\"%s\"" % targetName,
                "-archivePath",
                output_dir + os.sep + targetName
            ])

            self._run_cmd(command)
            app_path = os.path.join(output_dir, "%s.xcarchive" % targetName)
            ipa_cmd = "xcodebuild -exportArchive -archivePath " + app_path + " -exportPath " + output_dir + " -exportOptionsPlist " + ios_project_dir + os.sep + "ExportAppstore.plist"
            ...
            if specify_vs_ver is None:
                ...
                if specify_vs_ver in required_versions:
                    ...
                    raise cocos.CCPluginError(
                        MultiLanguage.get_string('COMPILE_ERROR_WRONG_VS_VER_FMT', specify_vs_ver),
                        cocos.CCPluginError.ERROR_WRONG_ARGS)
    ...
    def build_linux(self):
        ...
        with cocos.pushd(build_dir):
            ...
            self._run_cmd('cmake -DCMAKE_BUILD_TYPE=%s -DDEBUG_MODE=%s %s' % (
            build_mode, debug_state, os.path.relpath(cmakefile_dir, build_dir)))
    ...
    def get_wp8_product_id(self, manifest_file):
        ...
        try:
            ...
            raise cocos.CCPluginError(
                MultiLanguage.get_string('COMPILE_ERROR_MANIFEST_PARSE_FAILED_FMT', manifest_file),
    ...
    def run(self, argv, dependencies):
        ...
        self._project.invoke_custom_step_script(cocos_project.Project.CUSTOM_STEP_PRE_BUILD, target_platform,
                                                args_build_copy)
        ...
        self._project.invoke_custom_step_script(cocos_project.Project.CUSTOM_STEP_POST_BUILD, target_platform,
                                                args_build_copy)
