...
var pokerRule = (function () {
    ...
    var automaticTip = function(myCards, otherCards){
        ...
        if(otherCards && otherCards.length > 0){
            var paiType = pokerRule.judgeType(otherCards);
            var otherCardsArr = toCardsArr(otherCards);
            var card = otherCardsArr[0];

            if (paiType == pokerRule.CardType.c123) {
                //顺子
            ...
            }else if(paiType == pokerRule.CardType.c1122){
                //连对
                outArr = getLiandui(valueArr, retArr, card.weight);
                if(outArr && outArr.length >= otherCards.length){
                    return outArr.splice(0, otherCards.length);
                }
            }else if(paiType == pokerRule.CardType.c1112223344){
                //跑得快飞机
            ...
            }else if(paiType == pokerRule.CardType.c32){
                //三带几
                var cardwight = getMinWight(otherCardsArr, 3) || card.weight;
            ...
            }else if(gameData.wanfaDesp && gameData.wanfaDesp.indexOf('允许4带3') >= 0 && paiType == pokerRule.CardType.c4111){
                //四单三
            ...
            }else if(paiType == pokerRule.CardType.c2){
                //对
                outArr = getDanOrDui(valueArr, retArr, card.weight, 2);
                if(outArr){
                    return outArr;
                }
            }else if(paiType == pokerRule.CardType.c1){
                //单
            ...
            var bomb = getBomb(valueArr, retArr, (paiType == pokerRule.CardType.c4) ? card.weight:null);
            if(bomb)  return bomb;
        }else{
            //跑得快飞机
