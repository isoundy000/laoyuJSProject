...
public class SpeexEncoder implements Runnable {
	...
	public SpeexEncoder(String fileName, int frequency) {
		super();
		speex.init();
	...
	public void run() {
		...
		// 启动writer线程写speex文件。
		SpeexWriter fileWriter = new SpeexWriter(fileName, frequency);
		...
		while (this.isRecording()) {
			...
			if (list.size() > 0) {
				synchronized (mutex) {
					...
					//log.info("after encode......................before=" + rawdata.size + " after=" + processedData.length + " getsize=" + getSize);
				}
				if (getSize > 0) {
